...
function buildMarkdownSummary(reports, opts = {}) {
    ...
    for (const r of fwReports) {
        if (!includeEmpty && (r.matchedAlerts || 0) === 0)
            continue;
        ...
        if (summary) {
            ...
            for (const r of rows) {
                if (!r.ghsaId) {
                    ...
                    if (m)
                        r.ghsaId = m[0].toUpperCase();
                }
                if (!r.cveIds || r.cveIds.length === 0) {
                    const cves = new Set();
                    const re = /CVE-\d{4}-\d{4,7}/gi;
                    let m;
                    while ((m = re.exec(r.ruleId)) !== null)
                        cves.add(m[0].toUpperCase());
                    if (cves.size)
                        r.cveIds = Array.from(cves).sort();
                }
            }
            const cweMap = {};
            for (const r of rows) {
            ...
            for (const cwe of sortedCWEs) {
                ...
                for (const a of list) {
                    ...
                    const cves = (a.cveIds || []).map((c) => `[${c}](https://nvd.nist.gov/vuln/detail/${c})`).join(', ');
    ...
    const summary = reports.find(r => r.type === 'summary');
    if (summary) {
        const dependabotRows = (summary.findings || []).filter((f) => f.source === 'dependabot');
        const map = {};
        for (const row of dependabotRows) {
            const ghsa = row.ghsaId;
            const cwes = Array.isArray(row.cwes) ? row.cwes : [];
            const cveIds = Array.isArray(row.cveIds) ? row.cveIds : [];
            for (const cve of cveIds) {
                if (!map[cve])
                    map[cve] = { ghsa: ghsa, cwes: new Set() };
...
function buildAlertURL(owner, repo, source, id) {
    if (!id && id !== 0)
        return '';
    switch (source) {
        case 'dependabot':
