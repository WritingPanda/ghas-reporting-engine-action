...
def main(
    ...
    table.add_row("Target", organization or enterprise or "")
    ...
    try:
        ...
        with Progress(
            ...
            if not alerts:
                console.print(
                    "[yellow]Warning: No alerts found for the specified criteria[/yellow]"
                )
            ...
            for fmt in requested_formats:
                reporter_class = reporter_classes.get(fmt)

                if reporter_class is None:
                    logger.warning(f"Unsupported format '{fmt}' skipped")
                    console.print(f"[yellow]⚠ Skipping unsupported format: {fmt}[/yellow]")
                    continue
