...
from ..api.models import Alert, AlertSummary
...
class DataAnalyzer:
    ...
    def _analyze_summary(self, alerts: List[Alert]) -> AlertSummary:
        ...
        # Count by repository
        repo_counts = Counter(alert.repository.full_name for alert in alerts)
        summary.by_repository = dict(repo_counts)
        
        # Count by rule
        rule_counts = Counter(alert.rule.id for alert in alerts)
        ...
        for alert in alerts:
            for cwe in alert.cwes:
                cwe_counts[cwe] += 1
        summary.by_cwe = dict(cwe_counts)
        
        # Date range
        if alerts:
            dates = [alert.created_at for alert in alerts]
            summary.date_range = {
                "earliest": min(dates),
                "latest": max(dates),
            }
    ...
    def _analyze_sans(self, alerts: List[Alert], all_cwes: set) -> Dict[str, Any]:
        ...
        sorted_counts = sorted(
            sans_counts.items(),
            key=lambda x: sans_mappings[x[0]]["rank"]
        )
    ...
    def _analyze_mitre_kev(self, alerts: List[Alert], all_cwes: set) -> Dict[str, Any]:
        ...
        sorted_counts = sorted(
            kev_counts.items(),
            key=lambda x: kev_mappings[x[0]]["rank"]
        )
    ...
    def _analyze_repositories(self, alerts: List[Alert]) -> Dict[str, Any]:
        ...
        repo_analysis = defaultdict(lambda: {
            "total_alerts": 0,
